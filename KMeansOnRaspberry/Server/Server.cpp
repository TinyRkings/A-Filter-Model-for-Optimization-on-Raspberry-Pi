...
int main(int argc,char *argv[]){
	...
	int points_size = sizeof(struct mycentroid)* number_points;
 	...
 	struct timeval tpstart,tpend, tpfilter, tpfiltered, tpfiltered_later,tpfeaturesend,assignpoints,assignpointsend,updateend,rec,recend;
	...
	gettimeofday(&tpstart,NULL);
 	if(initial_gate == 1)
 	{
 		...
 		while(loop<10){
 			...
 			tmp=tmp+1000000*(recend.tv_sec-rec.tv_sec)+recend.tv_usec-rec.tv_usec; //us
 					...
 					if (psnrV < psnrTriggerValue && psnrV)
 					{
        				...
        				features_time = features_time+ 1000000*(tpfeaturesend.tv_sec-tpfiltered_later.tv_sec)+tpfeaturesend.tv_usec-tpfiltered_later.tv_usec; //us      			
 						...
 						if(num_points == number_points)
 						{
							...
							///////////////////// global clusters ////////////////////
							printf("gate %d, %d points receive used time:%f us\n",initial_gate, number_points, tmp);
							tmp=0;
